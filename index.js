...
io.on('connection',async socket => {
    ...
    socket.on('message', async e=>{
        ...
        io.to(arrOfUid[e.toId]).emit('dmNotification', e = {
            id : e.toId,
            msg : e.msg
        })
    ...
    socket.on('update-server', async e =>{
        io.to('server-'+e.sId).emit('newUpdateServer', info ={
            sId : e.sId,
            sName : e.sName
      })
    })

    socket.on('delete-server', async e =>{
        io.to('server-' + e.sId).emit('deleteServer', args = {
            uId : e.uId
        })
    })
    ...
    socket.on('text-channel-msg', async e=>{
        ...
        io.to('server-' + e.sId).emit('textNotification', a = {
            senderId : e.senderId,
            tId : e.tId,
            msg : e.msg
        })
